module.exports = function(grunt) {
    ...
    var fileConfig = {
        ...
        app_files: {
            ...
            ts: [ './src/**/*.module.ts', 'src/**/*.ts', '!src/**/*.spec.ts' ],
            tsunit: [ 'src/**/*.spec.ts' ],

            appTemplates: [ 'src/app/**/*.tpl.html' ],
            commonTemplates: [ 'src/common/**/*.tpl.html' ],
            componentTemplates: [ 'src/**/*.tpl.html', '!src/app/**/*.tpl.html', '!src/common/**/*.tpl.html' ],
        ...
        vendor_files: {
            js: [
                ...
                'vendor/application-model/appModel.module.js',
                'vendor/application-model/appModelProvider.js',
    ...
    var taskConfig = {
        ...
        concat: {
            ...
            compile_js: {
                ...
                src: [
                    ...
                    '<%= html2js.app.dest %>',
                    '<%= html2js.common.dest %>',
                    '<%= html2js.component.dest %>',
                    'module.suffix'
                ],
                dest: '<%= compile_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.js'
            },
            // The 'compile_component_js' target concatenates app code together. (without vendor js code)
            compile_component_js: {
                options: {
                    banner: '<%= meta.banner %>'
                },
                src: [
                ...
                dest: '<%= component_dir %>/<%= pkg.name %>-<%= pkg.version %>.js'
            }
        ...
        ts: {
            ...
            options: {
                ...
                sourceRoot: './src/app',                // where to locate TypeScript files. [(default) '' == source ts location]
            ...
            // a particular target
            dev: {
              files: [{ src: ['src/app/**/*.ts'], dest: './build/src/app' }],
                //html: ['src/app/**/**.tpl.html'],  // The source html files, https://github.com/basarat/grunt-ts#html-2-typescript-support
                reference: 'build/src/app/reference.ts', // If specified, generate this file that you can use for your reference management
        ...
        uglify: {
            compile: {
                ...
                files: {
                    ...
                    '<%= concat.compile_component_js.dest %>': '<%= concat.compile_component_js.dest %>'
                }
            }
        },
        ...
        html2js: {
            ...
            component: {
                ...
                src: [ '<%= app_files.componentTemplates %>' ],
        ...
        delta: {
            ...
            /**
             * When our TyoeScript source files change, we want to run lint them and
             * run our unit tests.
             */
            tssrc: {
                files: [
                ...
                tasks: [ 'ts:dev', 'karma:unit:run', 'copy:build_appjs', 'index:build' ]
            },
            /**
    ...
    grunt.registerTask('build', [
        'clean:all', 'ts', 'html2js', 'jshint', 'coffeelint', 'coffee', 'less:build',
    ...
    // The 'compile' task gets your componet ready for deployment by concatenating and minifying your code.
    // Note - compile builds off of the build dir (look at concat:compile_js), so run grunt build before grunt compile
    grunt.registerTask('compile-component', [
        'less:compile', 'copy:compile_component_assets', 'concat:compile_component_js', 'uglify', 'index:compile'
    ]);
